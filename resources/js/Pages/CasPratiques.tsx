...
type Category = 'infrastructure' | 'fichiers' | 'tickets' | 'saisie' | 'crm' | 'portabilite' | 'webservice' | 'supervision';
type Domain = 'pnm' | 'mobi';
...
// ─── Cas #14 — Incident CRM pendant bascule ─────────────────────────────────

const casMobiIncidentBascule: CasPratique = {
  id: 'mobi-incident-crm-bascule',
  number: 14,
  domain: 'mobi',
...
// ─── Cas #15 — MSISDN non reaffecte ─────────────────────────────────────────

const casMobiMsisdnBloque: CasPratique = {
  id: 'mobi-msisdn-non-reaffecte',
  number: 15,
  domain: 'mobi',
...
// ─── Cas #16 — WSMobiMaster ne repond pas ────────────────────────────────────

const casMobiWsMobiMaster: CasPratique = {
  id: 'mobi-wsmobimaster-down',
  number: 16,
  domain: 'mobi',
...
// ─── Cas #17 — Ecart PortaDB vs MasterCRM ────────────────────────────────────

const casMobiEcartPortaCrm: CasPratique = {
  id: 'mobi-ecart-portadb-crm',
  number: 17,
  domain: 'mobi',
...
const TAG_COLORS: Record<string, 'default' | 'primary' | 'secondary' | 'error' | 'info' | 'success' | 'warning'> = {
  ...
  MasterCRM: 'primary',
  'Porta entrante': 'info',
  'Porta sortante': 'warning',
  Status: 'secondary',
  WSMobiMaster: 'primary',
  SoapUI: 'info',
  Connectivite: 'warning',
...
export default function CasPratiques() {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [search, selectedDomain, selectedCategory, selectedSeverity]);

  const counts = useMemo(() => {
    const bySeverity: Record<Severity, number> = { critique: 0, majeur: 0, mineur: 0 };
    const byCategory: Record<Category, number> = { infrastructure: 0, fichiers: 0, tickets: 0, saisie: 0, crm: 0, portabilite: 0, webservice: 0, supervision: 0 };
    const byDomain: Record<Domain, number> = { pnm: 0, mobi: 0 };
    CAS_PRATIQUES.forEach((c) => {
      bySeverity[c.severity]++;
      byCategory[c.category]++;
      byDomain[c.domain]++;
    });
    return { bySeverity, byCategory, byDomain };
  }, []);
    ...
    <DashboardLayout>
      ...
      <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1200, mx: 'auto' }}>
        ...
        <Stack spacing={1} sx={{ mb: 3 }}>
          <Typography variant="h4">Cas Pratiques</Typography>
          <Typography variant="body2" color="text.secondary">
            {CAS_PRATIQUES.length} procedures documentees issues de cas reels. Filtrez par domaine, gravite ou categorie.
          </Typography>
        </Stack>
