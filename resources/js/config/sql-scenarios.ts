    ...
    scenarios: [
      {
        ...
        description:
          "SELECT * retourne toutes les colonnes d'une table. La table porta_operateur contient les operateurs telecoms des Antilles-Guyane (Digicel, Orange, SFR, etc.) avec leur code, nom, statut et coordonnees.",
      ...
      },
      {
        ...
        sql: 'SELECT code, description FROM porta_code_ticket\nORDER BY code;',
      },
      {
        id: 'deb-03',
    ...
    scenarios: [
      {
        ...
        label: 'Premier JOIN',
        description:
          "JOIN relie deux tables par une colonne commune. Ici, porta_portage et porta_etat via portage_etat. L'alias 'p' et 'e' raccourcissent les noms de table.",
      ...
      },
      {
        ...
        description:
          "Le ticket 500 est un refus de portage. Cette requete JOIN porta_portage_data et porta_code_reponse pour classer les motifs du plus frequent au moins frequent.",
      ...
      },
      {
        ...
        description:
          "Jointure avec porta_portage pour afficher le MSISDN. La table porta_portage_historique retrace les changements d'etat. ORDER BY date retrace la chronologie complete.",
        concepts: ['JOIN', 'WHERE subquery', 'historique'],
        sql: "SELECT ph.id, p.portage_msisdn AS msisdn,\n       ph.etat, ph.date, ph.commentaire\nFROM porta_portage_historique ph\nJOIN porta_portage p ON p.id = ph.id_portage\nWHERE p.portage_msisdn = (\n  SELECT portage_msisdn FROM porta_portage LIMIT 1\n)\nORDER BY ph.date;",
      },
      {
        id: 'int-10',
      ...
      },
      {
        ...
        description:
          "La table porta_transition definit les changements d'etat autorises. Les colonnes etat_initial et etat_final contiennent les noms d'etat (varchar). On peut joindre porta_etat pour obtenir le code associe.",
  ...
  investigation: {
    ...
    scenarios: [
      ...
      },
      {
        ...
        description:
          "IN (10, 20) filtre sur plusieurs codes reponse. 10 = delai trop court, 20 = erreur format. On joint porta_portage pour recuperer le MSISDN et le RIO du portage concerne.",
      ...
      },
      {
        ...
        sql: "SELECT p.portage_msisdn, p.portage_date_souhaitee,\n       fd.ferryday, e.etat_name AS etat\nFROM porta_portage p\nJOIN porta_ferryday fd\n  ON fd.ferryday = p.portage_date_souhaitee::date\nJOIN porta_etat e ON e.id = p.portage_etat\nWHERE fd.is_active = true;",
      },
      {
        id: 'inv-08',
        label: 'Erreurs techniques (code_reponse = 20)',
        description:
          "Le code_reponse 20 signale une erreur de format dans le traitement. Ces erreurs necessitent une intervention manuelle. On joint porta_portage pour obtenir le MSISDN concerne.",
      ...
      },
      {
        ...
        description:
          "Detecte les portages dont l'etat est 'termine' ou 'accepte' mais sans aucune entree dans porta_portage_historique. L'absence signale une desynchronisation entre les tables.",
        concepts: ['LEFT JOIN absence', 'coherence inter-tables'],
        sql: "SELECT p.id, p.portage_msisdn,\n       p.portage_date_souhaitee, e.etat_name AS etat\nFROM porta_portage p\nJOIN porta_etat e ON e.id = p.portage_etat\nLEFT JOIN porta_portage_historique ph\n  ON ph.id_portage = p.id\nWHERE ph.id IS NULL\n  AND e.etat_name IN ('termine', 'accepte')\nORDER BY p.portage_date_souhaitee DESC;",
      },
      {
        id: 'inv-12',
