...
- `docs/pnm/pnm-dockerisation-mep.md` — Cadrage de la dockerisation des MEP Porta (initiative Willy/PIL-media) : conteneurisation PortaWs (Tomcat/.war) + PortaSync (PHP/cron), principe (images taguees, registre, config externalisee, rollback), benefices (parite d'env, fin des oublis de dependance type #5239), actions Digicel (cadrage/tests/ops/process), points a clarifier.
